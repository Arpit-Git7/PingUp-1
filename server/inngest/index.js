...
const sendNewConnectionRequestReminder=inngest.createFunction(
    ...
    async({event,step})=>{
        ...
        await step.run('send-connection-request-reminded',async()=>{
            const connection=await Connection.findById(connectionId).populate('from_user_id to_user_id')
...
const sendNotificationofUnseenMessages=inngest.createFunction(
    ...
    async({step})=>{
        ...
        for(const userId in unseenCount){
            ...
            const subject=`Tou have ${unseenCount[userId]} unseen messages`;
            const body=`
            <div style="font-family:Arial,sans-serif;padding:20px;">
                <h2>Hi ${user.full_name},<h2>
                <p>You have ${unseenCount[userId]} unseen messages</p>
                <p>Click <a href="${process.env.FRONTEND_URL}/messages" style="color:#10b981;">here</a> to view the message<p>
                <br/>
                <p>Thanks,<br/>PingUp - Stay Connected</p>
            </div>`;
            await sendEmail({
