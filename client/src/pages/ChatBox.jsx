...
const ChatBox = () => {
  ...
  const loadUser = async () => {
    if (connections.length > 0) {
      const existingUser = connections.find(c => c._id === userId)
      if (existingUser) {
      ...
      <div className='px-4'>
          <div className='flex items-center gap-3 pl-5 p-1.5 bg-white w-full max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5'>
            ...
            <label htmlFor='image'>
              {
                image 
                ? <img src={URL.createObjectURL(image)} className='h-8 rounded' /> :<ImageIcon className='size-7 text-gray-400 cursor-pointer'/>
              }
              <input type='file' id='image' accept='iamge/*' hidden onChange={(e)=>setImage(e.target.files[0])}/>
            </label>
